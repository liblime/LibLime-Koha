...
sub CreateCourseReserve {
    ...
    unless ($result->{course_id}) {
        ...
        $sth2->execute($course_reserve->{course_id},$course_reserve->{itemnumber},$course_reserve->{staff_note},$course_reserve->{public_note},$course_reserve->{itemtype},$course_reserve->{ccode},$course_reserve->{location},$course_reserve->{branchcode},$item->{itemtype},$item->{ccode},$item->{location},$item->{homebranch});
        $sth2->finish;
