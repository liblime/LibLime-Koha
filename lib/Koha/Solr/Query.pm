...
has opac => ( is => 'ro', isa => 'Bool', default => 1 );
has options => ( is => 'rw', isa => 'HashRef' );
#has query => ( is => 'rw', isa => 'WebService::Solr::Query' );
has query => ( is => 'rw', isa => 'Str' );
has cgi => ( is => 'rw', isa => 'CGI' );
has uri => ( is => 'rw', isa => 'Str' );
has limits => ( is => 'rw', isa => 'ArrayRef' );
#has _qp => ( is => 'rw', isa => 'Search::QueryParser', handles => 'parse' );
has _parsed_query => ( is => 'rw', isa => 'ArrayRef' );
...
sub _build_query_from_cgi{
    ...
    # set simple query params so masthead can rebuild form elements.
        ...
        #  This regex won't do what we want on quoted queries.
   ...
   # $self->_parse_query_string($query); 
    $self->query($query);

    # Assemble options
     # for now just single sort.
    my $sort = $cgi->param('sort');
    if(!$sort && C4::Context->preference('OPACdefaultSortField') && C4::Context->preference('OPACdefaultSortField') ne 'score'){
        $sort = C4::Context->preference('OPACdefaultSortField')." ".C4::Context->preference('OPACdefaultSortOrder')
    }
...
#method _build_query_from_url(Str $url){
    # parses url-encoded search query.
